"""
main.py — Backend FastAPI del Bloque D.

Expone el pipeline de entrenamiento RL como una API web:
- Consulta de advisors (histórico + reward shaping) en vivo, antes de entrenar.
- Lanzar un entrenamiento real como subprocess.
- Streaming en vivo del progreso (stdout) vía Server-Sent Events.
- Consulta del resultado final una vez termina.

Reutiliza el mismo código que ya usa run_local_pipeline.py: no se
duplica lógica de negocio, solo se expone vía HTTP.
"""
...
def get_history():
    """
    Devuelve todas las corridas históricas disponibles: las del pipeline
    real (data/processed/historical_results_clean.csv) y las del paper
    (data/raw/historical_results_paper.csv), mezcladas y ordenadas por fecha.
    """
    import pandas as pd
