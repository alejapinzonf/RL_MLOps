...
def generate_run_id(scenario: str) -> str:
    ...
    return f"paper_real_q_learning_{scenario}_{timestamp}"


def build_q_table(grid_size: int, n_actions: int) -> np.ndarray:
    """
    Q-table densa indexada por (dx, dy, dxo, dyo, accion), igual que en
    Code.py: el rango de cada componente relativa va de -(grid_size-1)
    a (grid_size-1).
    """
    span = 2 * grid_size - 1
    return np.zeros((span, span, span, span, n_actions), dtype=np.float32)


def state_to_index(state: tuple[int, int, int, int], grid_size: int) -> tuple[int, int, int, int]:
    offset = grid_size - 1
    dx, dy, dxo, dyo = state
    return (dx + offset, dy + offset, dxo + offset, dyo + offset)
...
def train_one_episode(
    ...
    state = env.reset()
    index = state_to_index(state, env.grid_size)
    ...
    for step in range(1, env.max_steps + 1):
        action = choose_action(q_table, index, epsilon, env.n_actions)

        result = env.step(action)
        next_index = state_to_index(result.next_state, env.grid_size)

        old_value = q_table[index][action]
        best_next_value = np.max(q_table[next_index])

        td_target = result.reward + gamma * best_next_value * (not result.done)
        td_error = td_target - old_value

        q_table[index][action] = old_value + alpha * td_error
...
def save_new_run_csv(
    ...
    new_run = {
        ...
        "seed": args.seed,
        "notes": "real q-learning training run (paper environment/reward)",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrenamiento real Q-learning usando el entorno y reward del paper."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.80,
        help="Fracción del entrenamiento usada para decaer epsilon linealmente (igual a 'decay' en Code.py).",
    )
...
def main():
    ...
    q_table = build_q_table(args.grid_size, env.n_actions)
    ...
    print("\nEntrenamiento real Q-learning (paper env) completado.")
